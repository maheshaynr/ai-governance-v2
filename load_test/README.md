...
A standalone client for load testing the AI Governance API's three guardrail concerns
separately: PII masking, toxicity blocking, and the raw LLM path. All request content
is static, hand-written in `messages.json` — nothing is pulled from the database or
generated by an LLM at test time.
...
Every message in `messages.json` carries a `size` (small/medium/large) and `technique` tag, and (for `pii`/`toxicity`) an `expect` block the client checks the real response against — so a run reports both **latency** and **correctness**, not just speed.
...
### Full run — all three scenarios, one after another
...
|---|---|---|
| `--scenario` | `all` | `pii`, `toxicity`, `llm`, or `all` (sequential) |
...
| `--ollama-url` | from `config.py`'s `OLLAMA_URL` | Direct Ollama endpoint, `llm` scenario only |
| `--ollama-model` | from `config.py`'s `DEFAULT_LLM_MODEL` | Model name, `llm` scenario only |
...
- One row per request is written to `load_test/results/<scenario>_<timestamp>.csv` (columns: message id, technique, size, input size, api status, latency, pass/fail, error, plus `response_chars` for the `llm` scenario).
